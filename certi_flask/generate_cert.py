...
def generate_certificate(name, organisation):
    # Generate a unique ID for the certificate
    certificate_id = generate_certificate_id()

    # Generate the certificate PDF
    pdf = canvas.Canvas(f'certificates/{certificate_id}.pdf', pagesize=landscape(A4))
    pdf.drawImage('static/bg.jpg', 0, 0, width=842, height=595)

    # Set up the font and size for the certificate title
    pdf.setFont("heading", 48)
    pdf.drawCentredString(400, 400, "Certificate of Completion")

    # Set up the font and size for the recipient's name
    pdf.setFont("Helvetica", 36)
    pdf.drawCentredString(400, 300, f"Presented to {name}")

    gap = 30
    
    # Set up the font and size for the org
    pdf.setFont("Helvetica", 24)
    pdf.drawCentredString(400, 250 - 30, f"For donating to {organisation}")
    ...
    pdf.drawInlineImage(f"qrs/{certificate_id}.png", qr_x, qr_y, width=qr_width, height=qr_height)
